...
static INLINE void quantize(const __m256i* thr, const __m256i* qp, __m256i* c, const int16_t* iscan_ptr,
    ...
    const int nzflag = _mm256_movemask_epi8(_mm256_cmpgt_epi16(abs_coeff, *thr));
...
// AVX2 only exposes 32x32->64 multiplication for even lanes, so handle the
// odd lanes by moving them into the even positions instead of emulating a
...
static INLINE void quantize_qm(const __m256i* thr, const __m256i* qp, __m256i* c, const int16_t* iscan_ptr,
    ...
    if (EB_LIKELY(~nzflag)) {
        ...
        q = mul_shift_u32(q, tmp, AOM_QM_BITS + 16 - log_scale);
