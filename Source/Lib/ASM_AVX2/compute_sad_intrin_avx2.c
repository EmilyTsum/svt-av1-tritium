...
static INLINE uint32_t sad8x4_one_ref_sse2(__m128i src01, __m128i src23, const uint8_t* ref, int ref_stride) {
    __m128i ref01 = _mm_loadl_epi64((const __m128i*)ref);
    ref01 = _mm_castpd_si128(
        _mm_loadh_pd(_mm_castsi128_pd(ref01), (const double*)(const void*)(ref + ref_stride)));
    __m128i ref23 = _mm_loadl_epi64((const __m128i*)(ref + 2 * ref_stride));
    ref23 = _mm_castpd_si128(
        _mm_loadh_pd(_mm_castsi128_pd(ref23), (const double*)(const void*)(ref + 3 * ref_stride)));

    __m128i sad = _mm_add_epi64(_mm_sad_epu8(src01, ref01), _mm_sad_epu8(src23, ref23));
    sad         = _mm_add_epi32(sad, _mm_srli_si128(sad, 8));
    return (uint32_t)_mm_cvtsi128_si32(sad);
}
...
void svt_aom_sad8x4x4d_avx2(const uint8_t* src, int src_stride, const uint8_t* const ref[4], int ref_stride,
    ...
    src01 = _mm_castpd_si128(
        _mm_loadh_pd(_mm_castsi128_pd(src01), (const double*)(const void*)(src + src_stride)));
    __m128i src23 = _mm_loadl_epi64((const __m128i*)(src + 2 * src_stride));
    src23 = _mm_castpd_si128(
        _mm_loadh_pd(_mm_castsi128_pd(src23), (const double*)(const void*)(src + 3 * src_stride)));

    res[0] = sad8x4_one_ref_sse2(src01, src23, ref[0], ref_stride);
    res[1] = sad8x4_one_ref_sse2(src01, src23, ref[1], ref_stride);
    res[2] = sad8x4_one_ref_sse2(src01, src23, ref[2], ref_stride);
...
static AOM_FORCE_INLINE __m256i ext_eight_sad_16x16_block_avx2(
    const uint8_t* src, uint32_t src_stride, const uint8_t* ref, uint32_t ref_stride, uint32_t mv,
    uint32_t start_16x16_pos, uint32_t* p_best_sad_8x8, uint32_t* p_best_sad_16x16,
    uint32_t* p_best_mv8x8, uint32_t* p_best_mv16x16, bool sub_sad) {
...
void svt_ext_all_sad_calculation_8x8_16x16_avx2(uint8_t* src, uint32_t src_stride, uint8_t* ref, uint32_t ref_stride,
                                                ...
                                                uint32_t p_eight_sad16x16[16][8], bool sub_sad) {
    static const char offsets[16] = {0, 1, 4, 5, 2, 3, 6, 7, 8, 9, 12, 13, 10, 11, 14, 15};
    for (int y = 0; y < 4; ++y) {
        for (int x = 0; x < 4; ++x) {
            const uint32_t start_16x16_pos = offsets[4 * y + x];
            const __m256i sad16x16 = ext_eight_sad_16x16_block_avx2(src + 16 * y * src_stride + 16 * x,
                                                                     src_stride,
                                                                     ref + 16 * y * ref_stride + 16 * x,
                                                                     ref_stride,
                                                                     mv,
                                                                     start_16x16_pos,
                                                                     p_best_sad_8x8,
                                                                     ...
                                                                     p_best_mv16x16,
                                                                     sub_sad);
            _mm256_storeu_si256((__m256i*)p_eight_sad16x16[start_16x16_pos], sad16x16);
        }
    }
}
...
/* Horizontally search the minimum value in vector.
   ...
   */
#define avx2_find_min_value_pos(in, best_value, best_id)                            \
    {                                                                               \
        __m256i x        = _mm256_slli_epi32(in, 3); /* x = x<<3 | idx */           \
        x                = _mm256_add_epi32(x, idx_min_pos_prv);                    \
        const __m256i y  = _mm256_permute2f128_si256(x, x, 1);                     \
        const __m256i m1 = _mm256_min_epu32(x, y);                                 \
        const __m256i m2 = _mm256_permute4x64_epi64(m1, 5);                        \
        const __m256i m3 = _mm256_min_epu32(m1, m2);                               \
        const __m256i m4 = _mm256_shuffle_epi32(m3, 5);                            \
        const __m256i m  = _mm256_min_epu32(m3, m4);                               \
        const uint32_t packed_min = (uint32_t)_mm256_extract_epi32(m, 0);           \
        best_id                 = packed_min & 0x07;                                \
        best_value              = packed_min >> 3;                                 \
    }

void svt_ext_all_sad_calculation_8x8_16x16_32x32_64x64_avx2(
    ...
    uint32_t* p_best_mv16x16, uint32_t* p_best_sad_32x32, uint32_t* p_best_sad_64x64,
    uint32_t* p_best_mv32x32, uint32_t* p_best_mv64x64, uint32_t p_sad32x32[4][8], bool sub_sad) {
    avx2_find_min_pos_init();
    __m256i sad64 = _mm256_setzero_si256();
    const uint32_t mv_y = mv & 0xffff0000u;
    const uint32_t mv_x = mv & 0x0000ffffu;

    for (uint32_t group = 0; group < 4; ++group) {
        __m256i sad32 = _mm256_setzero_si256();
        const uint32_t qx = group & 1;
        const uint32_t qy = group >> 1;
        for (uint32_t local = 0; local < 4; ++local) {
            const uint32_t bx = local & 1;
            const uint32_t by = local >> 1;
            const uint32_t x  = (qx << 1) + bx;
            const uint32_t y  = (qy << 1) + by;
            const uint32_t start_16x16_pos = (group << 2) + local;
            const __m256i sad16x16 = ext_eight_sad_16x16_block_avx2(src + 16 * y * src_stride + 16 * x,
                                                                     src_stride,
                                                                     ref + 16 * y * ref_stride + 16 * x,
                                                                     ref_stride,
                                                                     ...
                                                                     p_best_mv16x16,
                                                                     sub_sad);
            sad32 = _mm256_add_epi32(sad32, sad16x16);
        }

        _mm256_storeu_si256((__m256i*)p_sad32x32[group], sad32);
        sad64 = _mm256_add_epi32(sad64, sad32);

        uint32_t min_sad, search_idx;
        avx2_find_min_value_pos(sad32, min_sad, search_idx);
        if (min_sad < p_best_sad_32x32[group]) {
            p_best_sad_32x32[group] = min_sad;
            p_best_mv32x32[group]   = mv_y | ((mv_x + search_idx) & 0x0000ffffu);
        }
    }

    uint32_t min_sad64, search_idx64;
    avx2_find_min_value_pos(sad64, min_sad64, search_idx64);
    if (min_sad64 < p_best_sad_64x64[0]) {
        p_best_sad_64x64[0] = min_sad64;
        p_best_mv64x64[0]   = mv_y | ((mv_x + search_idx64) & 0x0000ffffu);
    }
}
...
void svt_ext_eight_sad_calculation_32x32_64x64_avx2(const uint32_t p_sad16x16[16][8], uint32_t* p_best_sad_32x32,
    ...
    uint32_t min_a, min_b, min_c, min_d, min_e;
    ...
    const __m256i sad64 = _mm256_add_epi32(_mm256_add_epi32(sad32_a, sad32_b),
                                           _mm256_add_epi32(sad32_c, sad32_d));

    avx2_find_min_value_pos(sad32_a, min_a, si_a);
    avx2_find_min_value_pos(sad32_b, min_b, si_b);
    avx2_find_min_value_pos(sad32_c, min_c, si_c);
    avx2_find_min_value_pos(sad32_d, min_d, si_d);
    avx2_find_min_value_pos(sad64, min_e, si_e);

    const uint32_t mv_y = mv & 0xffff0000u;
    const uint32_t mv_x = mv & 0x0000ffffu;
#define UPDATE_BEST_SAD_MV(min_sad, search_idx, best_sad, best_mv)       \
    do {                                                                 \
        if ((min_sad) < (best_sad)) {                                    \
            (best_sad) = (min_sad);                                      \
            (best_mv)  = mv_y | ((mv_x + (search_idx)) & 0x0000ffffu);   \
        }                                                                \
    } while (0)

    UPDATE_BEST_SAD_MV(min_a, si_a, p_best_sad_32x32[0], p_best_mv32x32[0]);
    UPDATE_BEST_SAD_MV(min_b, si_b, p_best_sad_32x32[1], p_best_mv32x32[1]);
    UPDATE_BEST_SAD_MV(min_c, si_c, p_best_sad_32x32[2], p_best_mv32x32[2]);
    UPDATE_BEST_SAD_MV(min_d, si_d, p_best_sad_32x32[3], p_best_mv32x32[3]);
    UPDATE_BEST_SAD_MV(min_e, si_e, p_best_sad_64x64[0], p_best_mv64x64[0]);
#undef UPDATE_BEST_SAD_MV
}
